...
class BiSeNetV2(nn.Module):
    ...
    def __init__(self, n_classes, output_aux=True):
        ...
        ## changed for combine it with Gated-SCNN

        self.head = SegmentHead(128, 1024, 64, up_factor=8, aux=False)
        if self.output_aux:
            self.aux2 = SegmentHead(16, 128, 2048, up_factor=4)
            self.aux3 = SegmentHead(32, 128, 512, up_factor=8)
            self.aux4 = SegmentHead(64, 128, 1024, up_factor=16)
            self.aux5_4 = SegmentHead(128, 128, 2048, up_factor=32)
...
if __name__ == "__main__":
    ...
    model = torch.nn.DataParallel(BiSeNetV2(n_classes=6)).cuda()
